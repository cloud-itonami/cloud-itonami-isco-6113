(ns nurseryops.actor
  "NurseryOperationsActor — the ISCO-08 6113 nursery/horticultural-site
  scheduling/logistics coordination actor as a
  `langgraph.graph/state-graph` (ADR-2607121000 / CLAUDE.md Actors
  section). One graph run = one nursery-site-coordination operation
  ...
  housebuilder.actor for the physical-safety-domain shape (this
  ...
  The unconditional invariant: the Nursery Operations Advisor can
  never directly commit an operating record the NurseryOperationsGovernor
  ...
  regardless of disposition path. This actor coordinates NURSERY
  SCHEDULING/LOGISTICS ONLY — it never performs cultivation work or
  applies agrochemicals itself."
...
(defn build-graph
  "Build a compiled NurseryOperationsActor graph. `store` implements
  `nurseryops.store/Store`. `advisor` implements
  `nurseryops.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:grower-id (:grower-id request)
                                    :op (:op proposal)
                                    :nursery-id (:nursery-id proposal)
                                    :payload proposal}]
